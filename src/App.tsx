...
export default function App() {
  ...
  // App Download / PWA Installation States & Handlers
  ...
  const handleInstallApp = async () => {
    if (!deferredPrompt) {
      toast.info("Trình duyệt của bạn sẽ kích hoạt hộp thoại cài đặt nếu được hỗ trợ. Hoặc bạn có thể thêm thủ công vào màn hình chính.");
      return;
    }
    try {
  ...
  const handleCopyAppUrl = () => {
    ...
    navigator.clipboard.writeText(appUrl).then(() => {
      ...
      toast.success("Đã sao chép liên kết tải ứng dụng!");
      setTimeout(() => setInstallCopied(false), 2000);
    }).catch(() => {
      toast.error("Không thể tự động sao chép. Vui lòng tự sao chép URL trên trình duyệt!");
    });
  };

  const downloadIOSProfile = () => {
    const appUrl = window.location.origin;
    // Generate valid Apple mobileconfig plist for Web Clip
...
<dict>
	...
	<array>
		<dict>
			...
			<true/>
			<key>Label</key>
			<string>Shopee VIP</string>
			<key>PayloadDescription</key>
			<string>Cài đặt lối tắt Shopee VIP cực nhanh lên màn hình chính</string>
			<key>PayloadDisplayName</key>
			<string>Shopee VIP</string>
			<key>PayloadIdentifier</key>
			<string>com.shopee.vip.webclip</string>
			<key>PayloadType</key>
			<string>com.apple.webclip.managed</string>
			<key>PayloadUUID</key>
			<string>a780ad5b-801b-4efd-b940-1ece25fcbfbc</string>
	...
	<string>Shopee VIP</string>
	<key>PayloadIdentifier</key>
	<string>com.shopee.vip</string>
	<key>PayloadRemovalDisallowed</key>
	<false/>
	<key>PayloadType</key>
	<string>Configuration</string>
	<key>PayloadUUID</key>
	<string>b81ad50a-1111-4444-8888-2ece25fcbfba</string>
	<key>PayloadVersion</key>
	<integer>1</integer>
</dict>
</plist>`;
    ...
    link.href = url;
    link.download = 'shopeevip.mobileconfig';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    
    // Popup a sweet modal notification telling the user exactly how to install this profile in the settings app.
    toast.success("Đã tải File cấu hình iPhone thành công!", { duration: 5000 });
    alert("👉 HƯỚNG DẪN CÀI ĐẶT TRÊN IPHONE:\n\n1. Anh vừa tải xuống file cấu hình di động 'shopeevip.mobileconfig'.\n2. Bây giờ anh hãy thoát ra màn hình chính, mở ứng dụng \"Cài đặt\" (Settings) của iPhone.\n3. Anh sẽ thấy dòng chữ \"Đã tải về hồ sơ\" (Profile Downloaded) hiển thị ngay dưới tên tài khoản iCloud của anh.\n4. Hãy chạm vào dòng đó và bấm nút \"Cài đặt\" (Install) ở góc trên bên phải, nhập mật khẩu màn hình của anh nếu máy hỏi.\n5. Xong rồi! Biểu tượng app 'Shopee VIP' sẽ xuất hiện ngay lập tức trên màn hình chính của anh!");
  };
  ...
  const [categories, setCategories] = useState<{ name: string, image: string, count: number }[]>(DEFAULT_CATEGORIES);
  ...
  const fetchMeta = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Fallback to local calculated statistics
        setMeta({
          totalProducts: 59583,
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Fallback to local static categories
  ...
  const fetchDiverseProducts = async () => {
    try {
      ...
      // Pure client-side static index fallback
      const indexToUse = await fetchSearchIndex();
      if (indexToUse && indexToUse.length > 0) {
        const mapped = indexToUse.map(item => ({
          id: item.i,
          name: item.n,
          category: item.c,
        ...
        const shuffled = [...mapped].sort(() => Math.random() - 0.5).slice(0, 50);
        setDiverseProducts(shuffled);
      }
    } catch (e) {
  ...
  const fetchProducts = async (pageNum = 1, isLoadMore = false, customCategory?: string, customSearch?: string, customLimit?: number, customSort?: string) => {
    ...
    const mapFromIndex = (item: any): Product => ({
      id: item.i,
      name: item.n,
      category: item.c,
      discountPrice: item.p,
    ...
    const fetchProductsFromLocalIndex = async (pNum: number, isLoad: boolean, cat: string, query: string, lim: number, srt: string) => {
      try {
        ...
        if (indexToUse && indexToUse.length > 0) {
          ...
          if (query) {
            const normalizedSearch = removeVietnameseTones(query).toLowerCase().trim();
            const queryWords = normalizedSearch.split(/\s+/).filter(w => w.length > 0);
            const rawQueryLower = query.toLowerCase().trim();
            const rawQueryWords = rawQueryLower.split(/[^a-z0-9A-ZàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]+/g).filter(w => w.length > 0);

            let candidates = indexToUse;
            if (cat && cat !== 'Tất cả') {
              candidates = indexToUse.filter((p: any) => p.c === cat);
            }

            const runScoringOnDemand = (itemsList: any[]) => {
              return itemsList.map((item: any) => {
                const rawNameLower = (item.n || '').toLowerCase();
                const unsignedName = item.n_n || '';
                const unsignedCategory = removeVietnameseTones(item.c || '').toLowerCase();
                const cleanName = unsignedName.replace(/[^a-z0-9]/g, ' ').replace(/\s+/g, ' ').trim();
                const paddedUnsigned = ' ' + cleanName + ' ';
                const rawCleanName = rawNameLower.replace(/[^a-z0-9A-ZàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]/g, ' ').replace(/\s+/g, ' ').trim();
                const paddedRaw = ' ' + rawCleanName + ' ';

                let queryScore = 0;
                let matchedWholeWordsCount = 0;
                let matchedPrefixWordsCount = 0;

                queryWords.forEach((word) => {
                  const isShortWord = word.length <= 4;
                  if (paddedUnsigned.includes(' ' + word + ' ')) {
                    matchedWholeWordsCount++;
                    queryScore += 1000;
                  } else if (!isShortWord && paddedUnsigned.includes(' ' + word)) {
                    matchedPrefixWordsCount++;
                    queryScore += 400;
                  }
                });

                if (rawNameLower.includes(rawQueryLower)) {
                  queryScore += 1500;
                }
                const rScore = item.rs ? parseFloat(String(item.rs).replace(',', '.')) : 4.8;
                const sCount = item.ns || 0;
                const popularityBoost = Math.min(sCount / 2000, 50) + (rScore * 5);

                return { item, score: queryScore + popularityBoost, matchedWholeWordsCount, matchedPrefixWordsCount };
              });
            };

            let scoredCandidates = runScoringOnDemand(candidates);
            let filteredCandidates = scoredCandidates.filter((cand: any) => cand.matchedWholeWordsCount > 0 || cand.matchedPrefixWordsCount > 0 || cand.score >= 1500);
            filteredCandidates.sort((a: any, b: any) => b.score - a.score);
            results = filteredCandidates.map((cand: any) => cand.item);
          } else {
            results = cat && cat !== 'Tất cả'
              ? indexToUse.filter((p: any) => p.c === cat)
              : [...indexToUse];
          }
          ...
          const paginatedResults = mappedResults.slice((pNum - 1) * lim, pNum * lim);
          ...
          setHasMore(mappedResults.length > pNum * lim);
          setIsLoading(false);
          if (mappedResults.length > 0) setShowDemo(false);
          return true;
        }
      } catch (err) {
        console.error('Local index fallback failed:', err);
      }
      return false;
    };
    ...
    try {
      ...
      if (currentSearch || currentSort !== 'newest') {
        ...
        if (indexToUse && indexToUse.length > 0) {
          ...
          if (currentSearch) {
            const normalizedSearch = removeVietnameseTones(currentSearch).toLowerCase().trim();
            const queryWords = normalizedSearch.split(/\s+/).filter(w => w.length > 0);

            // Raw Vietnamese search query words to check signed matches
            const rawQueryLower = currentSearch.toLowerCase().trim();
            const rawQueryWords = rawQueryLower.split(/[^a-z0-9A-ZàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]+/g).filter(w => w.length > 0);

            let candidates = indexToUse;
            let isCategoryFiltered = false;
            if (currentCategory && currentCategory !== 'Tất cả') {
              candidates = indexToUse.filter((p: any) => p.c === currentCategory);
              isCategoryFiltered = true;
            }

            const runScoring = (itemsList: any[]) => {
              return itemsList.map((item: any) => {
                const rawNameLower = (item.n || '').toLowerCase();
                const unsignedName = item.n_n || '';
                const unsignedCategory = removeVietnameseTones(item.c || '').toLowerCase();

                // Clean name of non-alphanumeric to find whole words with simple spacers
                const cleanName = unsignedName.replace(/[^a-z0-9]/g, ' ').replace(/\s+/g, ' ').trim();
                const paddedUnsigned = ' ' + cleanName + ' ';
                ...
                queryWords.forEach((word) => {
                  ...
                  // 1. Exact Whole Word match (highest priority per word)
                  if (paddedUnsigned.includes(' ' + word + ' ')) {
                    matchedWholeWordsCount++;
                    queryScore += 1000;
                    
                    // original accented word match for accent-favoring bonus
                    const originalWordIndex = queryWords.indexOf(word);
                    if (originalWordIndex !== -1 && originalWordIndex < rawQueryWords.length) {
                      const rawWord = rawQueryWords[originalWordIndex];
                      if (paddedRaw.includes(' ' + rawWord + ' ')) {
                        queryScore += 300; // Accent-matching bonus!
                      }
                    }
                  } 
                  // 2. Prefix Match of a word (only allowed for longer words, length > 4)
                  else if (!isShortWord && paddedUnsigned.includes(' ' + word)) {
                    matchedPrefixWordsCount++;
                    queryScore += 400; // Half-weight for prefix word match
                  }
                });

                const totalWordCount = queryWords.length;
                const wholeWordMatchRatio = totalWordCount > 0 ? (matchedWholeWordsCount / totalWordCount) : 0;
                const prefixWordMatchRatio = totalWordCount > 0 ? (matchedPrefixWordsCount / totalWordCount) : 0;
                
                // Huge bonus for completing whole word matches for ALL search terms
                if (wholeWordMatchRatio === 1) {
                  queryScore += 2500;
                } else if (wholeWordMatchRatio + prefixWordMatchRatio === 1) {
                  queryScore += 1200; // All matches are whole or prefix
                } else if (wholeWordMatchRatio >= 0.5) {
                  queryScore += 400;
                }

                // 3. Exact/Substring Phrase matching of the entire query
                if (rawNameLower.includes(rawQueryLower)) {
                  const phraseIndex = rawNameLower.indexOf(rawQueryLower);
                  const beforeChar = phraseIndex > 0 ? rawNameLower[phraseIndex - 1] : ' ';
                  const afterChar = phraseIndex + rawQueryLower.length < rawNameLower.length ? rawNameLower[phraseIndex + rawQueryLower.length] : ' ';
                  const isWordBounded = /[^a-z0-9A-ZàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]/.test(beforeChar) && 
                                        /[^a-z0-9A-ZàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđĐ]/.test(afterChar);

                  queryScore += isWordBounded ? 3000 : 1500;
                  
                  if (rawNameLower.startsWith(rawQueryLower)) {
                    queryScore += 1000;
                  }
                } else if (unsignedName.includes(normalizedSearch)) {
                  const phraseIndex = unsignedName.indexOf(normalizedSearch);
                  const beforeChar = phraseIndex > 0 ? unsignedName[phraseIndex - 1] : ' ';
                  const afterChar = phraseIndex + normalizedSearch.length < unsignedName.length ? unsignedName[phraseIndex + normalizedSearch.length] : ' ';
                  const isWordBounded = /[^a-z0-9]/.test(beforeChar) && /[^a-z0-9]/.test(afterChar);

                  queryScore += isWordBounded ? 2000 : 1000;
                  
                  if (unsignedName.startsWith(normalizedSearch)) {
                    queryScore += 600;
                  }
                }

                // Add consecutive match bonus for words ordered correctly in the name
                let lastFoundIndex = -1;
                let consecutiveMatches = 0;
                queryWords.forEach((word) => {
                  const indexInName = unsignedName.indexOf(word);
                  if (indexInName !== -1) {
                    if (lastFoundIndex !== -1 && indexInName > lastFoundIndex) {
                      consecutiveMatches++;
                    }
                    lastFoundIndex = indexInName;
                  }
                });
                queryScore += consecutiveMatches * 150;

                // 4. Category relevance
                if (unsignedCategory.includes(normalizedSearch)) {
                  queryScore += 2000; // High score if matching category name
                } else {
                  let catMatches = 0;
                  queryWords.forEach(word => {
                    if (unsignedCategory.includes(word)) catMatches++;
                  });
                  if (queryWords.length > 0) {
                    queryScore += (catMatches / queryWords.length) * 800;
                  }
                }

                // 5. Popularity/rating tie-breaker (small enough to not override matches)
                const rScore = item.rs ? parseFloat(String(item.rs).replace(',', '.')) : 4.8;
                const sCount = item.ns || 0;
                const popularityBoost = Math.min(sCount / 2000, 50) + (rScore * 5); // Max 75

                const totalScore = queryScore + popularityBoost;
                ...
                return {
                  ...
                  matchedPrefixWordsCount,
                  wholeWordMatchRatio,
                  prefixWordMatchRatio
                };
              });
            };

            let scoredCandidates = runScoring(candidates);

            // Filter out candidates with zero keyword matching to prevent slop from showing
            let filteredCandidates = scoredCandidates.filter((cand: any) => {
              return cand.matchedWholeWordsCount > 0 || cand.matchedPrefixWordsCount > 0 || cand.score >= 1500;
            });

            // If we filtered with category, but got 0 results, relax category filter and search globally!
            if (filteredCandidates.length === 0 && isCategoryFiltered) {
              scoredCandidates = runScoring(indexToUse);
              filteredCandidates = scoredCandidates.filter((cand: any) => {
                return cand.matchedWholeWordsCount > 0 || cand.matchedPrefixWordsCount > 0 || cand.score >= 1500;
              });
            }

            // If we still got 0 results, let's relax and do fuzzy fallback using Fuse
            if (filteredCandidates.length === 0) {
              ...
              const fuseRes = localFuse.search(currentSearch);
              filteredCandidates = fuseRes.map((r: any) => {
                const item = r.item;
                const rScore = item.rs ? parseFloat(String(item.rs).replace(',', '.')) : 4.8;
                const sCount = item.ns || 0;
                const popularityBoost = Math.min(sCount / 200, 100) + (rScore * 5);
                const queryScore = (1 - (r.score || 0)) * 1000;
                return {
                  item,
                  score: queryScore + popularityBoost,
                  matchedWholeWordsCount: 1,
                  matchRatio: 0.5
                };
              });
            }

            // Sort descending by calculated score (highest score = most relevant!)
            filteredCandidates.sort((a: any, b: any) => b.score - a.score);

            // Strip metadata to get raw items
            results = filteredCandidates.map((cand: any) => cand.item);
          } else {
          ...
          // Interleave categories to ensure rich product diversity if looking at "Tất cả" category
          // ONLY apply this if no search query is active to maintain strict relevance sorting!
          if (!currentSearch && (!currentCategory || currentCategory === 'Tất cả') && currentSort !== 'price_asc' && currentSort !== 'price_desc') {
            const groupedByCat: Record<string, any[]> = {};
            results.forEach((item: any) => {
              const cat = item.c || 'Khác';
              if (!groupedByCat[cat]) groupedByCat[cat] = [];
              groupedByCat[cat].push(item);
            });
            
            const categoryNames = Object.keys(groupedByCat);
            ...
            while (hasMoreItems) {
              ...
              for (const cat of categoryNames) {
                if (index < groupedByCat[cat].length) {
                  interleaved.push(groupedByCat[cat][index]);
                  hasMoreItems = true;
                }
              }
              index++;
          ...
          const success = await fetchProductsFromLocalIndex(pageNum, isLoadMore, currentCategory, currentSearch, currentLimit, currentSort);
          if (success) return;
        }
      }
      ...
      // Fallback to local static index as static file fetch failed
      const fallbackSuccess = await fetchProductsFromLocalIndex(pageNum, isLoadMore, currentCategory, currentSearch, currentLimit, currentSort);
      ...
      // Last resort fallback to local static index
      const lastResortSuccess = await fetchProductsFromLocalIndex(pageNum, isLoadMore, currentCategory, currentSearch, currentLimit, currentSort);
      if (lastResortSuccess) return;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-gradient-to-r from-shopee via-orange-500 to-red-500 text-white py-2 text-[11px] md:text-xs font-semibold shadow-sm border-b border-shopee/10">
        <div className="max-w-7xl mx-auto px-4 flex flex-row justify-between items-center gap-2">
          <div className="flex items-center gap-2">
            <span className="bg-white text-shopee text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase animate-pulse">App</span>
            <span className="hidden sm:inline">Trải nghiệm Shopee Official Mall cực đỉnh ngay trên điện thoại!</span>
            <span className="inline sm:hidden">Cài đặt Shopee Mall về điện thoại!</span>
          </div>
          <button 
            id="btn-main-install"
            onClick={() => setShowAppInstallModal(true)} 
            className="flex items-center gap-1 bg-white hover:bg-orange-50 text-shopee font-bold px-3 py-1 rounded-full text-[10px] md:text-xs transition-all shadow-sm active:scale-95 cursor-pointer"
          >
            ...
            >
              <div className="flex flex-wrap items-center gap-3 mb-6">
                ...
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-pink-500 via-rose-500 to-purple-600 text-white rounded-full text-sm shadow-xl shadow-pink-200/50 animate-pulse hover:scale-110 transition-transform cursor-default">
                  ...
                  <span className="font-stylish text-lg tracking-wider drop-shadow-md"> Tổng Hợp Hot Deal Giảm Giá Mạnh 24/7</span>
                </div>
              </div>
              <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold text-gray-900 leading-[1.15] mb-8 tracking-tight uppercase">
                HOT DEAL <span className="text-shopee">TỔNG HỢP CHÍNH HÃNG</span>
              </h1>
        ...
        <section id="deals" className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="relative min-h-[400px]">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2 sm:gap-2.5 product-grid">
      ...
      <footer id="footer" className="bg-gray-900 text-white pt-16 pb-8">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  <span className="font-bold text-xl tracking-tight text-shopee">
                    Shopee VIP
                  </span>
                </div>
              </div>
              <p className="text-gray-400 text-sm leading-relaxed font-sans">
                Shopee VIP là trang web tổng hợp những sản phẩm HOT nhất trên thị trường và các chương trình giảm giá mạnh cực sâu Hot Deal ưu đãi, cam kết mang đến tay khách hàng các sản phẩm chính hãng 100% uy tín hàng đầu.
              </p>
            </div>
            ...
            <div>
              <h4 className="font-bold mb-6">Tải Ứng Dụng Di Động</h4>
              <p className="text-xs text-gray-400 mb-4 leading-relaxed">
                Tải ứng dụng Shopee VIP về điện thoại để mua sắm mượt mà, tiện lợi và nhận nhiều ưu đãi đặc quyền hơn.
              </p>
              <button
                onClick={() => setShowAppInstallModal(true)}
                className="flex items-center gap-2 bg-shopee hover:bg-shopee-hover text-white text-xs font-bold px-4 py-3 rounded-xl transition-all shadow-md cursor-pointer active:scale-95"
              >
  ...
  <AnimatePresence>
    {showAppInstallModal && (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        ...
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-shopee to-orange-600 text-white p-6 relative">
            <button
              onClick={() => setShowAppInstallModal(false)}
              className="absolute top-4 right-4 p-1.5 bg-white/20 hover:bg-white/30 rounded-full transition-colors text-white cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
            <div className="flex items-center gap-3">
              <div className="bg-white text-shopee p-2 rounded-2xl shadow-md">
                <ShoppingBag className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-lg font-extrabold tracking-tight">Tải Ứng Dụng Di Động</h3>
                <p className="text-xs text-orange-100 font-medium">Trải nghiệm mua sắm mượt mà, tiện lợi nhất</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 overflow-y-auto max-h-[75vh] space-y-5">
            ...
            {/* Action QR and PWA dynamic trigger */}
            <div className="bg-orange-50/50 p-4 rounded-2xl border border-orange-100 flex flex-col sm:flex-row items-center gap-5">
              <div className="bg-white p-2.5 rounded-2xl border border-orange-200 shadow-inner flex-shrink-0">
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=130x130&color=ee4d2d&data=${encodeURIComponent(window.location.origin)}`}
                  alt="QR Code"
              ...
              <div className="flex-1 text-center sm:text-left space-y-2">
                <h4 className="text-sm font-bold text-gray-900 flex items-center justify-center sm:justify-start gap-1.5">
                  <QrCode className="w-4 h-4 text-shopee" /> Quét QR Hoặc Copy Link
                </h4>
                <p className="text-[11px] text-gray-500 leading-relaxed">
                  Bật camera máy ảnh điện thoại của anh để quét mã QR bên cạnh để truy cập nhanh trên điện thoại, hoặc sao chép nhanh liên kết dưới đây.
                </p>
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 pt-1 font-sans">
                  <button
                    onClick={handleCopyAppUrl}
                    className="flex items-center gap-1.5 bg-white hover:bg-gray-100 border border-gray-200 text-gray-700 font-bold px-3 py-2 rounded-xl text-xs transition-all cursor-pointer shadow-sm"
                  >
                    {installCopied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-gray-500" />}
                    Copy Link Web App
                  </button>
                  {deferredPrompt && (
                    <button
                      onClick={handleInstallApp}
                      className="flex items-center gap-1.5 bg-shopee hover:bg-shopee-hover text-white font-bold px-3 py-2 rounded-xl text-xs transition-all cursor-pointer shadow-sm"
                    >
                      <Smartphone className="w-3.5 h-3.5" />
                      Cài đặt trực tiếp
                    </button>
            ...
            <div className="space-y-3">
              <div className="flex p-1 bg-gray-100 rounded-xl">
                <button
                  ...
                  className={`flex-1 text-center py-2.5 text-[11px] sm:text-xs font-bold rounded-xl transition-all cursor-pointer ${
                ...
                <button
                  onClick={() => setActiveInstallTab('store')}
                  className={`flex-1 text-center py-2.5 text-[11px] sm:text-xs font-bold rounded-xl transition-all cursor-pointer ${
                    activeInstallTab === 'store'
                      ? 'bg-white text-shopee shadow-sm'
                      : 'text-gray-500 hover:text-gray-900'
                  }`}
                >
              ...
              {activeInstallTab === 'ios' ? (
                <div className="bg-gray-50 p-4 rounded-2xl space-y-4 border border-gray-100 text-left">
                  <div className="bg-white p-4 rounded-xl border border-red-100 shadow-sm space-y-2 text-center">
                    <span className="text-[10px] uppercase tracking-wider font-extrabold text-shopee bg-red-50 px-2 py-0.5 rounded-full">Khuyên dùng cho iPhone</span>
                    <h5 className="text-sm font-bold text-gray-800">Cài đặt 1-chạm thông qua Cấu hình (Profile)</h5>
                    <p className="text-[11px] text-gray-500">Anh không cần tìm nút bấm chia sẻ phức tạp, chỉ cần nhấn nút đỏ dưới đây:</p>
                    <button
                      onClick={downloadIOSProfile}
                      className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-3 rounded-2xl text-xs transition-all cursor-pointer shadow-md flex items-center justify-center gap-2 uppercase tracking-wide transform hover:scale-102 active:scale-98"
                    >
                      <Download className="w-4 h-4 animate-bounce" />
                      Tải File Cài Đặt Cho iPhone (iOS)
                    </button>
                  </div>

                  <div className="border-t border-gray-200/60 pt-3 space-y-2.5">
                    <h6 className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Cách phụ (Thủ công qua Safari):</h6>
                    <div className="flex items-start gap-3">
                      <span className="w-5 h-5 bg-shopee/10 text-shopee text-[10px] font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">1</span>
                      <p className="text-xs text-gray-700 leading-relaxed">
                        Mở trình duyệt <strong>Safari</strong> trên iPhone và truy cập vào trang web của anh.
                      </p>
                    </div>
                    <div className="flex items-start gap-3">
                      <span className="w-5 h-5 bg-shopee/10 text-shopee text-[10px] font-bold rounded-full flex-shrink-0 mt-0.5" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>2</span>
                      <p className="text-xs text-gray-700 leading-relaxed flex items-center gap-1 flex-wrap">
                        Nhấn vào nút <strong>Chia sẻ</strong> <Share2 className="w-3.5 h-3.5 inline mx-0.5 text-shopee" /> ở dưới cùng thanh công cụ Safari.
                      </p>
                    </div>
                    <div className="flex items-start gap-3">
                      <span className="w-5 h-5 bg-shopee/10 text-shopee text-[10px] font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">3</span>
                      <p className="text-xs text-gray-700 leading-relaxed">
                        Kéo xuống dưới rồi tìm và chạm vào dòng <strong>"Thêm vào MH chính"</strong> (Add to Home Screen). Sau đó, nhấn nút <strong>"Thêm"</strong> ở góc trên bên phải là xong!
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-2xl space-y-4 border border-gray-100 text-left">
                  <div className="bg-white p-4 rounded-xl border border-orange-100 shadow-sm space-y-3">
                    <div className="text-center space-y-1">
                      <span className="text-[10px] uppercase tracking-wider font-extrabold text-white bg-orange-500 px-2 py-0.5 rounded-full">CỰC KỲ DỄ DÀNG</span>
                      <h5 className="text-sm font-bold text-gray-800">Tự generate file .APK cài đặt cực kỳ đơn giản</h5>
                      <p className="text-[11px] text-gray-500 leading-relaxed">
                        Vì hệ thống của anh là Web App cao cấp (PWA), anh có thể đóng gói website của mình thành file <strong>.APK cài đặt riêng</strong> chỉ mất 1 phút bằng công cụ miễn phí **PWABuilder** (của Microsoft phát hành):
                      </p>
                    </div>
                    
                    <div className="space-y-3 pt-2">
                      <div className="flex items-start gap-2.5">
                        <span className="w-4 h-4 bg-orange-100 text-orange-600 text-[10px] font-black rounded-full flex items-center justify-center mt-0.5">1</span>
                        <div className="flex-1 space-y-1">
                          <p className="text-[11px] text-gray-700">Sao chép đường link website của anh dưới đây:</p>
                          <div className="flex gap-1.5 max-w-full">
                            <input 
                              type="text" 
                              readOnly 
                              value={window.location.origin} 
                              className="bg-gray-100 border border-gray-200 text-gray-600 text-[10px] px-2 py-1 rounded-md flex-1 font-mono focus:outline-none overflow-x-auto" 
                            />
                            <button
                              onClick={() => {
                                navigator.clipboard.writeText(window.location.origin);
                                toast.success("Đã sao chép link web app!");
                              }}
                              className="bg-orange-500 hover:bg-orange-600 text-white font-bold text-[10px] px-2 py-1.5 rounded-md cursor-pointer flex items-center gap-1 active:scale-95"
                            >
                      ...
                      <div className="flex items-start gap-2.5">
                        <span className="w-4 h-4 bg-orange-100 text-orange-600 text-[10px] font-black rounded-full flex items-center justify-center mt-0.5">2</span>
                        <div className="flex-1">
                          <p className="text-[11px] text-gray-700 leading-relaxed">
                            Truy cập trang web đóng gói ở link dưới, dán link vừa copy vào ô dán URL và bấm nút <strong className="text-orange-600">Start</strong>:
                          </p>
                          <a 
                            href="https://www.pwabuilder.com" 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1 text-orange-600 hover:text-orange-700 font-bold text-xs underline mt-1"
                          >
                            Mở Trang PWABuilder.com <Download className="w-3.5 h-3.5 inline" />
                          </a>
                        </div>
                      </div>

                      <div className="flex items-start gap-2.5">
                        <span className="w-4 h-4 bg-orange-100 text-orange-600 text-[10px] font-black rounded-full flex items-center justify-center mt-0.5">3</span>
                        <p className="text-[11px] text-gray-700 leading-relaxed">
                          Màn hình tiếp theo hiện ra, anh chọn mục <strong className="text-orange-600">Android</strong> và bấm <strong className="font-bold">Generate</strong> để hệ thống tự động xuất file cài đặt <strong>.APK</strong> để anh tải xuống. Anh có thể trực tiếp chia sẻ file .APK đó lên các hội nhóm, gửi Zalo để bất kỳ máy Android nào cũng cài đặt được trong 1 giây!
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-gray-200/60 pt-3 space-y-2.5">
                    <h6 className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Cách cài trực tiếp thông thường (Qua Chrome):</h6>
                    <div className="flex items-start gap-3">
                      <span className="w-5 h-5 bg-shopee/10 text-shopee text-[10px] font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">1</span>
                      <p className="text-xs text-gray-700 leading-relaxed font-semibold">
                        Sử dụng trình duyệt <strong>Google Chrome</strong> trên điện thoại Android của anh để truy cập website này.
                      </p>
                    </div>
                    <div className="flex items-start gap-3">
                      <span className="w-5 h-5 bg-shopee/10 text-shopee text-[10px] font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">2</span>
                      <p className="text-xs text-gray-700 leading-relaxed">
                        Nhấn vào biểu tượng <strong>nút 3 chấm đứng</strong> ở phía góc trên cùng bên phải trình duyệt Chrome.
                      </p>
                    </div>
                    <div className="flex items-start gap-3">
                      <span className="w-5 h-5 bg-shopee/10 text-shopee text-[10px] font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">3</span>
                      <p className="text-xs text-gray-700 leading-relaxed">
                        Chạm vào dòng <strong>"Thêm vào màn hình chính"</strong> (hoặc <strong>"Cài đặt ứng dụng"</strong>). Sau đó xác nhận thêm là app sẽ hiện ra màn hình chính ngay!
                      </p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="bg-gray-50 p-4 rounded-2xl space-y-3.5 border border-gray-100 text-left">
                  <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm space-y-2">
                    <div className="flex items-center gap-1.5 text-blue-600 font-extrabold text-xs">
                      <Rocket className="w-4 h-4 animate-pulse" /> ĐƯA APP LÊN CH PLAY & APP STORE
                    </div>
                    <h5 className="text-sm font-bold text-gray-800">Trả lời: HOÀN TOÀN ĐƯỢC! Anh có thể đưa lên chính thức</h5>
                    <p className="text-[11px] text-gray-500 leading-relaxed">
                      Để người dùng tìm kiếm và tải ứng dụng <strong>Shopee VIP</strong> trực tiếp trên kho ứng dụng CH Play và App Store, anh cần thực hiện quy trình chính thức sau:
                    </p>
                  </div>

                  <div className="space-y-3">
                    <div className="flex gap-2">
                      <div className="w-5 h-5 bg-blue-100 text-blue-600 font-black text-[10px] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">1</div>
                      <div className="flex-1 text-xs">
                        <strong className="text-gray-900 block font-bold">Đóng gói mã nguồn (Packaging):</strong>
                        Dán link website của anh vào công cụ đóng gói chuyên dung như <a href="https://www.pwabuilder.com" target="_blank" rel="noopener noreferrer" className="text-blue-600 underline font-semibold">PWABuilder</a>. Chọn xuất bản file cho "Google Play Store" (hệ thống trả về file định dạng <code className="bg-gray-100 px-1 py-0.5 rounded text-red-600 font-mono text-[10px]">.aab</code>) và cho Apple "App Store" (trả về file định dạng <code className="bg-gray-100 px-1 py-0.5 rounded text-red-600 font-mono text-[10px]">.ipa</code> hoặc package xcode).
                      </div>
                    </div>

                    <div className="flex gap-2">
                      <div className="w-5 h-5 bg-blue-100 text-blue-600 font-black text-[10px] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">2</div>
                      <div className="flex-1 text-xs">
                        <strong className="text-gray-900 block font-bold">Tạo tài khoản nhà phát triển (Developer Account):</strong>
                        <ul className="list-disc pl-4 mt-1 space-y-1 text-gray-600">
                          <li><strong>Google Play Developer Console (Android):</strong> Phí là <strong>$25 USD (khoảng 600.000 VNĐ)</strong> mua một lần dùng trọn đời.</li>
                          <li><strong>Apple Developer Program (iOS):</strong> Phí là <strong>$99 USD/năm (khoảng 2.400.000 VNĐ)</strong> gia hạn mỗi năm.</li>
                        </ul>
                      </div>
                    </div>

                    <div className="flex gap-2">
                      <div className="w-5 h-5 bg-blue-100 text-blue-600 font-black text-[10px] rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">3</div>
                      <div className="flex-1 text-xs">
                        <strong className="text-gray-900 block font-bold">Nộp app lên Store & Chờ Xét Duyệt:</strong>
                        Tải các file đóng gói ở Bước 1 lên tài khoản nhà phát triển đã tạo ở Bước 2, bổ sung thông tin mô tả giới thiệu hấp dẫn, ảnh chụp màn hình điện thoại thực tế, sau đó gửi duyệt. CH Play sẽ duyệt trong 24h - 3 ngày, App Store sẽ duyệt trong 1 - 5 ngày. Khi duyệt xong, app của anh sẽ xuất hiện công khai trên toàn cầu!
                      </div>
                    </div>
                  </div>

                  <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl mt-2 flex items-start gap-2 font-sans">
                    <Info className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />
                    <p className="text-[11px] text-amber-800 leading-relaxed">
                      <strong>Hỗ trợ từ chuyên viên:</strong> Bọn em luôn sẵn sàng hỗ trợ anh chuẩn bị file đóng gói chất lượng cao, viết mô tả chuẩn SEO, và tư vấn từng bước đăng ký tài khoản để ứng dụng được đưa lên CH Play & App Store một cách thành công và chuyên nghiệp nhất! Anh cứ nhắn em nhé.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="bg-gray-50 p-4 border-t border-gray-100 flex justify-end">
            <button
              onClick={() => setShowAppInstallModal(false)}
              className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold px-5 py-2.5 rounded-xl text-xs transition-all cursor-pointer"
            >
              Đóng lại
            </button>
          </div>
