...
async function saveStaticData(products: any[]) {
  ...
  const productsTempDir = path.join(dataDir, 'products_temp');
  
  debugLog(`[STATIC EXPORT] Starting atomic export of ${products.length} products to ${dataDir}`);
  
  try {
    // Ensure directories exist
    if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
    
    // Always use a clean temp directory for writing pages
    if (fs.existsSync(productsTempDir)) {
      fs.rmSync(productsTempDir, { recursive: true, force: true });
    }
    fs.mkdirSync(productsTempDir, { recursive: true });

    // 1. Save Meta atomically
    ...
    const tempMetaPath = path.join(dataDir, 'meta.json.tmp');
    fs.writeFileSync(tempMetaPath, JSON.stringify(meta, null, 2));
    fs.renameSync(tempMetaPath, path.join(dataDir, 'meta.json'));

    // 2. Save Categories atomically
    ...
    const tempCategoriesPath = path.join(dataDir, 'categories.json.tmp');
    fs.writeFileSync(tempCategoriesPath, JSON.stringify(categoriesData, null, 2));
    fs.renameSync(tempCategoriesPath, path.join(dataDir, 'categories.json'));

    // 3. Save Search Index atomically
    ...
    const tempSearchIndexPath = path.join(dataDir, 'search-index.json.tmp');
    fs.writeFileSync(tempSearchIndexPath, JSON.stringify(searchIndex));
    fs.renameSync(tempSearchIndexPath, path.join(dataDir, 'search-index.json'));

    // 4. Save Paginated Products
    const pageSize = 692;
    
    // Helper to save pages in temp directory
    const savePages = (items: any[], subDir: string) => {
      const dir = path.join(productsTempDir, subDir);
    ...
    // Save "All" after mixing categories to ensure maximum diversity across all pages of "All"
    const interleaveProducts = (items: any[]) => {
      ...
      while (hasMore) {
        ...
        for (const cat of categoryNames) {
          if (index < grouped[cat].length) {
            result.push(grouped[cat][index]);
            hasMore = true;
          }
        }
        index++;
    ...
    const interleavedProducts = interleaveProducts(products);
    savePages(interleavedProducts, 'all');
    ...
    // Now swap productsTempDir to productsDir ATOMICALLY to ensure zero downtime
    const productsBackupDir = path.join(dataDir, 'products_old');
    if (fs.existsSync(productsBackupDir)) {
      fs.rmSync(productsBackupDir, { recursive: true, force: true });
    }

    if (fs.existsSync(productsDir)) {
      // Instantly swap existing folder to backup
      fs.renameSync(productsDir, productsBackupDir);
    }
    
    // Instantly promote temp folder to products folder
    fs.renameSync(productsTempDir, productsDir);

    // Clean up backup in the background/sync
    if (fs.existsSync(productsBackupDir)) {
      fs.rmSync(productsBackupDir, { recursive: true, force: true });
    }
    
    debugLog('[STATIC EXPORT] Atomic export completed successfully.');
    return true;
  } catch (error) {
    console.error('[STATIC EXPORT] Error:', error);
    // Cleanup temp folder if something went wrong
    if (fs.existsSync(productsTempDir)) {
      try { fs.rmSync(productsTempDir, { recursive: true, force: true }); } catch (e) {}
    }
    throw error;
  }
}
...
async function syncGoogleSheets(isManual = false) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Google Sheets trả về lỗi HTTP ${response.status}`);
    }

    const csvData = await response.text();
    
    console.log(`[SYNC] Response received. Length: ${csvData.length} bytes.`);
    console.log(`[SYNC] First 100 characters: ${csvData.substring(0, 100)}`);
    ...
    try {
      ...
      if (!usingFirestore) {
        ...
        if (hasRelevance && sort === 'newest') {
          ...
          if (!category || category === 'Tất cả') {
            // Diversify Best Sellers by partitioning by category and ordering by highest sold count inside each category
            sql = `
              SELECT * FROM (
                SELECT *, 
                ROW_NUMBER() OVER (
                  PARTITION BY category 
                  ORDER BY numericSoldCount DESC, (id * 17 % 1000003) DESC
                ) as rank
                FROM ${fromClause}
                ${whereClauses.length > 0 ? ' WHERE ' + whereClauses.join(' AND ') : ''}
              ) 
              ORDER BY rank, numericSoldCount DESC
            `;
          } else {
          ...
          if (!category || category === 'Tất cả') {
            // Diversify Newest products by partitioning by category and ordering by newest ID inside each category
            sql = `
              SELECT * FROM (
                SELECT *, 
                ROW_NUMBER() OVER (
...
async function startServer() {
  ...
  const server = app.listen(PORT, '0.0.0.0', async () => {
    ...
    // Force static files generation with the new interleaving/diversification only if they do not exist
    try {
      const dataDir = path.join(process.cwd(), 'public', 'data');
      ...
      // We only export if SQLite has a significant amount of data,
      // and we either don't have search-index.json or it is empty or incomplete.
      if (products.length > 1) {
        if (!fs.existsSync(searchIndexPath)) {
          ...
          try {
            ...
            if (!Array.isArray(indexData) || indexData.length < products.length) {
              needsExport = true;
            }
          } catch (e) {
            needsExport = true;
      ...
      } else if (products.length === 1 && !fs.existsSync(searchIndexPath)) {
        // Only export the single seeded product if there are absolutely no static files at all
        needsExport = true;
      }

      if (needsExport) {
        debugLog('[STARTUP] Static files missing or incomplete. Re-exporting static data with category-interleaving diversification...');
        await saveStaticData(products);
        debugLog('[STARTUP] Static data re-export completed successfully!');
      } else {
        debugLog('[STARTUP] Skipping re-export on startup. Valid static files already exist or SQLite database has only seed data.');
      }
    } catch (e: any) {
      console.error('[STARTUP] Failed to generate static data:', e.message);
    }
  });
